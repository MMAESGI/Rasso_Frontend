...
function onFormSubmit(event: FormSubmitEvent<LoginRequest>) {
...
<template>
  <div class="w-full h-full flex items-center justify-center py-12">
    <div
      class="shadow-xl rounded w-[90%] md:w-[500px] md:min-h-3/4 p-12 flex flex-col justify-between"
    >
      ...
      >
        ...
        <Button class="md:self-end md:px-6! mt-5" type="submit"
          >{{ t('connexion.login.title') }} <i class="pi pi-arrow-right"></i
        ></Button>
      </Form>
      <div class="text-center">
        {{ t('connexion.login.noAccount') }}
        <RouterLink to="/register" class="underline">{{
          t('connexion.register.title')
        }}</RouterLink>
      </div>
    </div>
  </div>
